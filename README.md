...
- **Authentication:** npm trusted publishing (OIDC). On npmjs.com, in the package settings under Trusted Publisher, choose GitHub Actions with user `OoJae`, repository `n8n-nodes-technocore`, workflow `publish.yml`, no environment. No `NPM_TOKEN` secret is needed; if one is set, the workflow also writes it to the npm config as a token fallback.
