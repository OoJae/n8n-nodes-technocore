...
const credentials: CredentialData = {
	...
	technocoreSigningKeyApi: { origin: ORIGIN, privateKeySeed: TEST_SEEDS.seed01, allowAiToolSigning: false },
};

function postedReply(room: string, record: IDataObject): FakeResponse {
	return {
		status: 200,
		body: serverJson({ room, count: 1, first_seq: record.seq, last_seq: record.seq, generation: 1, messages: [record], posted: record }, 1),
	};
}

async function run(params: IDataObject | IDataObject[], router: Router, extra: { nodeType?: string; continueOnFail?: boolean; credentials?: CredentialData } = {}) {
	const stub = makeExecuteFunctions({ params, router, credentials: extra.credentials ?? credentials, nodeType: extra.nodeType, continueOnFail: extra.continueOnFail });
...
describe('Technocore node description', () => {
	it('is usable as a tool and exposes Room and Note operations', () => {
		...
		const ops = d.properties.filter((p) => p.name === 'operation').flatMap((p) => p.options?.map((o) => (o as { value: string }).value));
...
describe('Room: Read', () => {
	it('reads the newest window with since and reports a gap', async () => {
		...
		room.postMany(30);
		const { items, requests } = await run({ resource: 'room', operation: 'read', room: 'lobby', limit: 10, since: 5, output: 'perMessage' }, () => ({
			status: 200,
			body: room.read(5, 10),
			headers: {},
		}));
		...
		expect(items[0].json).toMatchObject({ type: 'message', untrusted: true, signed: false, gapDetected: true });
	...
	it('batch output carries the gap range', async () => {
		const room = new FakeRoom('lobby');
		room.postMany(30);
		const { items } = await run({ resource: 'room', operation: 'read', room: 'lobby', limit: 10, since: 5, output: 'batch' }, () => ({ status: 200, body: room.read(5, 10) }));
		expect(items).toHaveLength(1);
		expect(items[0].json).toMatchObject({ type: 'batch', untrusted: true, count: 10, firstSeq: 21, lastSeq: 30, gapDetected: true, gap: { from: 6, to: 20 } });
	});
	...
	it('refuses an invalid room before sending anything', async () => {
		const { items, requests } = await run({ resource: 'room', operation: 'read', room: '../kv' }, () => ({ status: 200, body: '' }), { continueOnFail: true });
...
describe('Room: Post', () => {
	it('posts JSON {from, text} using the default nickname', async () => {
		const { items, requests } = await run({ resource: 'room', operation: 'post', room: 'lobby', text: 'hi there', nick: '' }, () =>
			postedReply('lobby', { seq: 7, ts: TEST_TS, from: 'n8n-bot', text: 'hi there' }),
		);
		expect(requests[0].sent).toMatchObject({ method: 'POST', url: `${ORIGIN}/r/lobby?format=json`, body: { from: 'n8n-bot', text: 'hi there' } });
		expect(items[0].json).toMatchObject({ type: 'posted', seq: 7, from: 'n8n-bot', signed: false, untrusted: true });
	});

	it('refuses unsigned posts to mailbox rooms and points to Post Signed', async () => {
		const error = await run({ resource: 'room', operation: 'post', room: 'mb-p-box', text: 'x' }, () => ({ status: 200, body: '' })).catch((e: unknown) => e);
	...
	it('surfaces the 422 duplicate refusal', async () => {
		const error = await run({ resource: 'room', operation: 'post', room: 'lobby', text: 'copy copy copy copy', nick: 'me' }, () => ({
			status: 422,
...
describe('Room: Post Signed', () => {
	const signedRouter: Router = (request) => {
		...
		return postedReply('lobby', { seq: 3, ts: TEST_TS, from: body.did, text: body.text, nonce: body.nonce, sig: body.sig });
	};

	it('sends only {text, context} from the node and the credential adds did/sig/nonce', async () => {
		const { items, requests } = await run({ resource: 'room', operation: 'postSigned', room: 'lobby', signedText: 'signed hello' }, signedRouter);
		...
		expect(items[0].json).toMatchObject({ type: 'posted', from: identity.did, signed: true, nonce: sent.nonce });
	...
	it('reports aiTool context from the tool variant and the credential refuses it by default', async () => {
		const error = await run({ resource: 'room', operation: 'postSigned', room: 'lobby', signedText: 'x' }, signedRouter, {
			nodeType: 'n8n-nodes-technocore.technocoreTool',
		}).catch((e: unknown) => e);
		expect((error as Error).message).toMatch(/AI agent tool are disabled/);

		const allowed = await run({ resource: 'room', operation: 'postSigned', room: 'lobby', signedText: 'x' }, signedRouter, {
			nodeType: 'n8n-nodes-technocore.technocoreTool',
			credentials: { ...credentials, technocoreSigningKeyApi: { ...credentials.technocoreSigningKeyApi, allowAiToolSigning: true } },
		});
	...
	it('retries once past a stale nonce reported by the server', async () => {
		...
		const router: Router = (request) => {
			...
			const body = request.body as Record<string, string>;
			if (calls === 1) {
				return { status: 400, body: `400 nonce ${body.nonce} is not greater than 9100000000000000000, the last one this key used in /r/lobby - count up\n` };
			}
			return signedRouter(request);
		};
		const { items, requests } = await run({ resource: 'room', operation: 'postSigned', room: 'lobby', signedText: 'again' }, router);
		expect(requests).toHaveLength(2);
		expect(requests[1].requested.body).toEqual({ text: 'again', context: 'workflow', nonceAfter: '9100000000000000000' });
	...
	it('does not retry other 400s', async () => {
		let calls = 0;
		const error = await run({ resource: 'room', operation: 'postSigned', room: 'lobby', signedText: 'x' }, () => {
			calls++;
...
describe('Note operations', () => {
	it('reads a note, stripping the untrusted banner', async () => {
		const { items, requests } = await run({ resource: 'note', operation: 'read', namespace: 'status', key: 'bot' }, () => ({
			status: 200,
			body: `${BANNER}\n\nall systems nominal\n`,
		}));
		expect(requests[0].sent.url).toBe(`${ORIGIN}/kv/status/bot`);
		expect(items[0].json).toEqual({ namespace: 'status', key: 'bot', found: true, untrusted: true, value: 'all systems nominal' });
	});

	it('returns found=false on 404 by default and fails when asked', async () => {
		const notFound = () => ({ status: 404, body: '404 no note status/bot\n' });
		const { items } = await run({ resource: 'note', operation: 'read', namespace: 'status', key: 'bot' }, notFound);
		expect(items[0].json).toEqual({ namespace: 'status', key: 'bot', found: false });
		await expect(run({ resource: 'note', operation: 'read', namespace: 'status', key: 'bot', notFound: 'error' }, notFound)).rejects.toBeInstanceOf(NodeApiError);
	});

	it('writes with if_absent / if conditions and surfaces a 409 with the current value', async () => {
		const ok = () => ({ status: 200, body: '{"ns": "status", "key": "bot", "bytes": 2, "ts": "t"}\n' });
		const absent = await run({ resource: 'note', operation: 'write', namespace: 'status', key: 'bot', value: 'up', condition: 'ifAbsent' }, ok);
		expect(absent.requests[0].sent).toMatchObject({ method: 'POST', url: `${ORIGIN}/kv/status/bot?format=json`, body: { value: 'up', if_absent: true } });
		expect(absent.items[0].json).toMatchObject({ written: true, bytes: 2 });

		const match = await run({ resource: 'note', operation: 'write', namespace: 'status', key: 'bot', value: 'up', condition: 'ifMatch', expected: 'down' }, ok);
		expect(match.requests[0].sent.body).toEqual({ value: 'up', if: 'down' });

		const conflict = await run({ resource: 'note', operation: 'write', namespace: 'status', key: 'bot', value: 'up', condition: 'ifAbsent' }, () => ({
			status: 409,
