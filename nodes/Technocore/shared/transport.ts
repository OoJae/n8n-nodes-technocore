...
import { classifyRefusal, parseConflictValue } from './responses';
...
/** A 409 quotes a note value of up to 8192 code points after its guidance; keep all of it. */
const MAX_CONFLICT_DESCRIPTION = 2000 + 2 * 8192;
 ...
 * value), so the description labels it untrusted. A 409's current value is also put, whole,
 * in the error's `context.currentValue`, so a workflow can rebase on it.
 */
export function refusalError(
	...
	const limit = refusal.kind === 'conflict' ? MAX_CONFLICT_DESCRIPTION : MAX_DESCRIPTION;
	const quoted =
		response.body.length > limit ? `${response.body.slice(0, limit)}...` : response.body;
	...
	if (refusal.kind === 'conflict') {
		const current = parseConflictValue(response.body);
		if (current !== null) {
			error.context.currentValue = current;
			error.context.untrusted = true;
		}
	}
	return error;
}
